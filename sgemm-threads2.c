...
void * threaded_sgemm(void * vp) {
  ...
  int chunk_size = p->m / PTHREAD_THREADS_MAX / dimMax * dimMax;
  for (int C_I = p->thread_number * chunk_size;
       C_I < ((p->thread_number + 1) * chunk_size);
       C_I++) {


    for (int C_K = 0; C_K < p->n/dimMax*dimMax; C_K+=dimMax){

      for (int C_J = 0; C_J <p->m/dimMax*dimMax; C_J+=dimMax){

        for (int j = C_J; j < C_J+dimMax; j+=4){
          for (int k = C_K; k < C_K+dimMax; k+=4){
            for (int i = (C_I * dimMax); i < (C_I + 1)*dimMax; i+=4){
	      ...
	      //printf("%d %d %d -- %d %d %d\n", C_I, C_K, C_J, j, k, i);
              ...
              __m128 a1 = _mm_loadu_ps(p->A + i +p->m*(k+0));
              __m128 a2 = _mm_loadu_ps(p->A + i +p->m*(k+1));
              __m128 a3 = _mm_loadu_ps(p->A + i +p->m*(k+2));
              __m128 a4 = _mm_loadu_ps(p->A + i +p->m*(k+3));

              __m128 b1x = _mm_loadu_ps(p->B + j+0 +p->m*(k+0));
              __m128 b2x = _mm_loadu_ps(p->B + j+0 +p->m*(k+1));;
              __m128 b3x = _mm_loadu_ps(p->B + j+0 +p->m*(k+2));;
              __m128 b4x = _mm_loadu_ps(p->B + j+0 +p->m*(k+3));;
            ...
            //Accumulate sum in C
            _mm_storeu_ps(p->C + i +p->m*(j+0), _mm_add_ps(c1, _mm_loadu_ps(p->C + i +p->m*(j+0))));
            _mm_storeu_ps(p->C + i +p->m*(j+1), _mm_add_ps(c2, _mm_loadu_ps(p->C + i +p->m*(j+1))));
            _mm_storeu_ps(p->C + i +p->m*(j+2), _mm_add_ps(c3, _mm_loadu_ps(p->C + i +p->m*(j+2))));
            _mm_storeu_ps(p->C + i +p->m*(j+3), _mm_add_ps(c4, _mm_loadu_ps(p->C + i +p->m*(j+3))));
