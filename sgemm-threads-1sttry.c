...
/* void (* f_threadQueue)(void *) = NULL; */

/* void aux_threadQueue() { */
/*   while (42) { */
/*     if (f_threadQueue) { */
/*       f_threadQueue(); */
/*       f_threadQueue = NULL; */
...
} threadForParamsCounter;

void aux_threadFor(void * vp) {
  threadForParams *p = vp;
  for (int i = p->min; i < p->max; ++i)
    p->f(p->arg);
}

void threadFor(void (*f)(void *), void * arg, int min, int max) {
  pthread_t t[PTHREAD_THREADS_MAX + 1];
  threadForParams p;
  int chunk_size = max / PTHREAD_THREADS_MAX;
  for (ssize_t thread_num = 0; thread_num < PTHREAD_THREADS_MAX; ++thread_num) {
    p = { thread_num * chunk_size, (thread_num + 1) * chunk_size, arg, f };
    if ((pthread_create(&t[thread_num], NULL, aux_threadFor, &p)) != 0)
...
} tranferDataFromOldMatricesParams;

void tranferDataFromOldMatrices(void * p) {
...
void sgemm(int m, int n, float * A, float * B, float * C) {
  ...
  if (m % dimMax || n % xmm_size){
    ...
    threadFor(tranferDataFromOldMatrices, , 0, n);
